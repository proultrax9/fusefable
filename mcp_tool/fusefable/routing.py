...
def _gateway_key(cfg: Config) -> str:
    return os.environ.get(cfg.api_key_env, "") or cfg.api_key


def _provider_key(sp) -> str:
...
def build_routes(cfg: Config, http: httpx.AsyncClient) -> list[tuple]:
    ...
    if cfg.mode == "gateway":
        ...
        pc = ProviderConfig(cfg.gateway_name, cfg.gateway_base_url, _gateway_key(cfg))
        prov = make_provider("openai_compat", pc, http)
        for model in cfg.models:
            routes.append((prov, model))
    else:
        for sp in cfg.providers:
            pc = ProviderConfig(sp.name, sp.base_url, _provider_key(sp))
